...
{
    public class SessionWorkspaceStatePersistenceService
    {
        ...
        public void SaveWorkspaceState(SessionWorkspaceState state, string path)
        {
            ...
            var recoveryState = new SessionWorkspaceRecoveryState
            {
                ...
                PendingScenarioSourcePath = state.PendingScenarioSourcePath,
        ...
        public SessionWorkspaceRecoveryState LoadWorkspaceState(string path)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(path);
        ...
        private static SessionWorkspaceRecoveryState LoadWorkspaceStateCore(string path)
        {
            var json = File.ReadAllText(path, Encoding.UTF8);

            try
            {
                var recoveryState = JsonSerializer.Deserialize<SessionWorkspaceRecoveryState>(json, SerializerOptions);
        ...
        private static void ValidateRecoveryState(SessionWorkspaceRecoveryState recoveryState)
        {
            if (recoveryState.CurrentFilePath is not null
                && !recoveryState.CurrentFilePath.EndsWith(SnapshotFileExtensions.TableSession, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException("Workspace state CurrentFilePath must use a table session snapshot extension.");
            }

            if (recoveryState.PendingScenarioSourcePath is not null
                && !recoveryState.PendingScenarioSourcePath.EndsWith(SnapshotFileExtensions.Scenario, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException("Workspace state PendingScenarioSourcePath must use a scenario snapshot extension.");
            }

            if (recoveryState.CurrentSnapshotFormat is not null
                && recoveryState.CurrentSnapshotFormat != SnapshotFormatKind.TableSessionSnapshot
                && recoveryState.CurrentSnapshotFormat != SnapshotFormatKind.ScenarioSnapshot)
            {
